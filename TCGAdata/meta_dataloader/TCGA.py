...
class TCGAMeta(Dataset):
    ...
    def __init__(self, data_dir=None, dataset_transform=None, transform=None, target_transform=None, download=False,
                 preload=True, min_samples_per_class=3, task_variables_file=None, gene_symbol_map_file=None):
    ...
    def __getitem__(self, index):
        ...
        """
        dataset = TCGATask(self.task_ids[index], transform=self.transform, target_transform=self.target_transform,
                           download=False, preloaded=self.preloaded, gene_symbol_map_file=self.gene_symbol_map_file)
...
def get_TCGA_task_ids(data_dir=None, min_samples_per_class=3, task_variables_file=None):
    ...
    for filename in os.listdir(os.path.join(data_dir, 'clinicalMatrices')):
        ...
        for task_variable in task_variables:
            ...
            num_samples_per_class_is_in_range = all(
                [num_samples > min_samples_per_class for num_samples in num_samples_per_label.values()])
